...
request_log = deque(maxlen=10)

example_github_payload = None
with open('example_data/example_github_payload.json') as f:
    example_github_payload = json.load(f)

request_log.append({
    'headers': {'foo': 'bar'},
    'body': example_github_payload,
})
...
class Webhook(object):
    ...
    def __init__(self, app, endpoint="/postreceive", secret=None):
        ...
        app.add_url_rule(rule='/', endpoint='/', view_func=self.pong, methods=["GET", "POST"])
    ...
    def pong(self):
        return render_template('index.html', requests=request_log), 200
